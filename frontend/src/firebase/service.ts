...
export async function updateDepartmentAction(
  ...
  // Backend role-based jurisdiction enforcement
  if (officerRole !== 'admin') {
    const roleToDeptMap: Record<string, DepartmentKey> = {
      library_officer: 'library',
      hostel_officer: 'hostel',
      sports_officer: 'sports',
      accounts_officer: 'accounts'
    };
    const authorizedDept = roleToDeptMap[officerRole];
    if (authorizedDept && authorizedDept !== deptKey) {
      throw new Error(`Unauthorized: Your credentials only authorize actions for the ${authorizedDept} department.`);
    }
  }
...
export async function registerStudentUser(data: StudentRegistrationData): Promise<AppUser> {
  ...
  if (!isAllowedStudentEmail(normEmail)) {
    throw new Error('Please use your official RGUKT university email address.');
  }
  if (data.password.length < 6) {
    throw new Error('Password must be at least 6 characters long.');
  }

  const cleanRollNo = data.studentId.trim().toUpperCase();
  let uid = `stud_${cleanRollNo.toLowerCase().replace(/[^a-z0-9]/g, '')}`;
  ...
  try {
    ...
    if (err.code === 'auth/email-already-in-use') {
      throw new Error('An account with this university email already exists. Please sign in instead.');
    }
    // If Email/Password is not enabled in Firebase Console, fallback gracefully to Firestore-based profile
    if (err.code === 'auth/operation-not-allowed' || err.message?.includes('operation-not-allowed')) {
      console.info('Firebase Auth Email/Password not enabled; using seamless preview student profile.');
    } else {
      throw err;
    }
  }
  ...
  try {
    await setDoc(doc(db, 'users', uid), sanitizeFirestoreData(appUser));
    await setDoc(doc(db, 'students', uid), sanitizeFirestoreData({
      uid,
...
export async function loginStudentUser(email: string, password: string): Promise<AppUser> {
  ...
  try {
    ...
    if (err.code === 'auth/operation-not-allowed' || err.message?.includes('operation-not-allowed')) {
      // If it is the default demo student, provide seamless login
      if (normEmail === DEFAULT_DEMO_STUDENT.email) {
        return {
          uid: 'demo_stud_r200048',
          email: DEFAULT_DEMO_STUDENT.email,
          displayName: DEFAULT_DEMO_STUDENT.displayName,
          role: 'student',
          rollNo: 'R200048',
          createdAt: new Date().toISOString()
        };
      ...
      // Check if student exists in Firestore (registered previously)
      try {
        const studSnap = await getDocs(query(collection(db, 'users'), where('email', '==', normEmail)));
        if (!studSnap.empty) {
          const userDoc = studSnap.docs[0].data() as AppUser;
          return {
            ...userDoc,
        ...
        console.warn('Student lookup note:', lookupErr);
      }

      // Auto-extract roll number from email (e.g., rahul.r200048@rguktrkv.ac.in -> R200048)
      const rollMatch = normEmail.match(/r[0-9]{6}/i);
      const derivedRoll = rollMatch ? rollMatch[0].toUpperCase() : 'R' + Math.floor(100000 + Math.random() * 900000);
      const namePart = normEmail.split('@')[0].replace(/[._0-9-]/g, ' ').trim();
      const derivedName = namePart ? namePart.replace(/\b\w/g, (c) => c.toUpperCase()) : 'RGUKT Student';

      return {
        uid: `stud_${derivedRoll.toLowerCase()}`,
        email: normEmail,
...
export async function registerAuthorityUser(data: AuthorityRegistrationData): Promise<AppUser> {
  ...
  let cred;
  let uid = `emp_${cleanEmpId.toLowerCase().replace(/[^a-z0-9]/g, '')}`;
  ...
  try {
    ...
    if (err.code === 'auth/email-already-in-use') {
      throw new Error('An authority account with this official email already exists. Please sign in instead.');
    }
    if (err.code === 'auth/operation-not-allowed' || err.message?.includes('operation-not-allowed')) {
      console.info('Firebase Auth Email/Password disabled; creating authority profile with local UID.');
    } else {
      throw err;
    }
  }
  ...
  try {
    ...
    console.warn('Authority profile sync note:', dbErr);
  }
...
export async function loginAuthorityUser(employeeId: string, password: string): Promise<AppUser> {
  ...
  try {
    ...
    if (err.code === 'auth/operation-not-allowed' || err.message?.includes('operation-not-allowed')) {
      ...
      if (demo) {
        return {
          uid: `demo_emp_${demo.employeeId.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
          email: demo.email,
      ...
      return {
        uid: officerMeta?.uid || `emp_${cleanEmpId.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
        email: officerEmail,
        displayName: officerMeta?.displayName || 'Verifying Officer',
        role: officerMeta?.role || 'department_head',
        employeeId: cleanEmpId,
        department: officerMeta?.department || 'library',
        createdAt: new Date().toISOString()
      };
    }
    ...
    if (demo && password === demo.password) {
      try {
        ...
        if (inner.code === 'auth/operation-not-allowed' || inner.message?.includes('operation-not-allowed')) {
          return {
            uid: `demo_emp_${demo.employeeId.toLowerCase().replace(/[^a-z0-9]/g, '')}`,
            email: demo.email,
            displayName: demo.displayName,
...
export async function resetUserPassword(email: string): Promise<void> {
  ...
  try {
    ...
    if (err.code === 'auth/operation-not-allowed' || err.message?.includes('operation-not-allowed')) {
      throw new Error(
        'Password reset via Email is disabled in your Firebase project. Please enable "Email/Password" in Firebase Console > Authentication > Sign-in method.'
      );
    }
    throw err;
  }
