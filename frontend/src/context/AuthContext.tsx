  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (fbUser: FirebaseUser | null) => {
      ...
      if (fbUser) {
        ...
        // If not authenticated in Firebase Auth, retain local/preview user if active
        setStudentUser((prev) => {
          if (prev && (prev.uid.startsWith('demo_') || prev.uid.startsWith('stud_'))) {
            return prev;
          }
          const saved = localStorage.getItem('clearpass_student_user');
          if (saved) {
        ...
        setAuthorityUser((prev) => {
          if (prev && (prev.uid.startsWith('demo_') || prev.uid.startsWith('emp_'))) {
            return prev;
          }
          const saved = localStorage.getItem('clearpass_authority_user');
          if (saved) {
