...
import { ClearanceRequest, AuditRecord, DigitalCertificate, DepartmentKey, UserRole } from './types';
import { Search, Award, ShieldCheck, CheckCircle2 } from 'lucide-react';

export type AppTab = 'landing' | 'home' | 'how-it-works' | 'my-clearance' | 'certificate' | 'register';
...
function MainApp() {
  ...
  // Navigation tab for student portal / register
  const [currentTab, setCurrentTab] = useState<AppTab>(() => {
    if (window.location.hash.includes('register') || window.location.pathname.includes('/register')) return 'register';
    if (window.location.hash.includes('my-clearance')) return 'my-clearance';
    if (window.location.hash.includes('certificate')) return 'certificate';
    if (window.location.hash.includes('how-it-works')) return 'how-it-works';
    return 'landing';
  });
  ...
  // Handle URL hash changes for /authority and /register routes
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (isReg) {
        ...
        setIsAuthorityRoute(false);
        if (window.location.hash.includes('my-clearance')) {
          setCurrentTab('my-clearance');
        } else if (window.location.hash.includes('certificate')) {
          setCurrentTab('certificate');
        } else if (window.location.hash.includes('how-it-works')) {
  ...
  // Student Registered -> Student Dashboard / Portal
  const handleStudentRegistered = () => {
    window.location.hash = 'my-clearance';
    setCurrentTab('my-clearance');
    setIsAuthorityRoute(false);
  };

  // Higher Authority Registered -> Appropriate Department or Authority Portal
  const handleAuthorityRegistered = (role: UserRole, department?: DepartmentKey) => {
    if (role === 'library_officer') {
      window.location.hash = 'authority/library';
      setAuthorityDept('library');
      setIsAuthorityRoute(true);
    } else if (role === 'hostel_officer') {
      window.location.hash = 'authority/hostel';
      setAuthorityDept('hostel');
      setIsAuthorityRoute(true);
    } else if (role === 'sports_officer') {
      window.location.hash = 'authority/sports';
      setAuthorityDept('sports');
      setIsAuthorityRoute(true);
    } else if (role === 'accounts_officer') {
      window.location.hash = 'authority/accounts';
      setAuthorityDept('accounts');
      setIsAuthorityRoute(true);
    } else {
      ...
      <main className="flex-1">
        {currentTab === 'register' && (
          <RegisterPage
            ...
            onAuthorityRegistered={handleAuthorityRegistered}
            onOpenSignIn={() => setShowSignInModal(true)}
            onNavigateHome={() => {
              window.location.hash = '';
              setCurrentTab('landing');
