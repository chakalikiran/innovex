...
interface LandingPageProps {
  ...
  onNavigateToTab: (tab: 'landing' | 'home' | 'how-it-works' | 'my-clearance' | 'certificate' | 'register') => void;
  onOpenSignIn?: () => void;
  onOpenRegister?: () => void;
...
export function LandingPage({
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      {/* 1. HERO SECTION: Full-Width Landscape Central Library Hero */}
      <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-2 sm:pt-4 pb-10 sm:pb-14">
        <div className="relative w-full overflow-hidden rounded-xl sm:rounded-2xl border border-black/10 dark:border-white/10 bg-slate-900 shadow-lg min-h-[460px] sm:min-h-[520px] md:min-h-[560px] lg:min-h-[600px] xl:min-h-[640px] flex items-center">
          {/* Central Library Photograph - Intelligent Responsive Crop */}
          <img
            src={rkvalleyCampusImg}
            alt="University Central Library — Academic Clearance Hub"
            referrerPolicy="no-referrer"
            className="absolute inset-0 w-full h-full object-cover object-[center_36%] sm:object-[center_38%] md:object-[center_40%] lg:object-[58%_42%] select-none pointer-events-none transition-all duration-300"
          />

          {/* Subtle Directional Neutral Overlay: Keeps text razor-sharp while preserving library vibrancy */}
          <div
            className="absolute inset-0 pointer-events-none bg-gradient-to-t from-black/85 via-black/45 to-black/15 sm:bg-gradient-to-r sm:from-black/85 sm:via-black/55 sm:via-45% sm:to-black/20 lg:from-black/85 lg:via-black/45 lg:via-52% lg:to-transparent"
            aria-hidden="true"
          />

          {/* Institutional Typography & Primary CTA Placed Directly Over Image */}
          <div className="relative z-10 w-full px-6 sm:px-10 md:px-12 lg:px-16 py-12 sm:py-16 lg:py-20 flex flex-col justify-center">
            <div className="max-w-xl lg:max-w-2xl text-left">
              <h1 className="text-3xl sm:text-5xl md:text-6xl lg:text-[66px] xl:text-[72px] leading-[1.03] tracking-tight font-light text-white mb-5 sm:mb-6">
                Your clearance.<br />
                <span className="italic font-serif serif font-normal text-white">One digital journey.</span>
              </h1>

              <p className="text-sm sm:text-base md:text-lg text-slate-200/95 leading-relaxed mb-8 sm:mb-10 max-w-xl font-normal">
                Complete your university No-Dues process digitally — request departmental verification, track every approval, and receive your certificate without visiting multiple offices.
              </p>
              ...
              <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                <button
                  ...
                  className="inline-flex items-center gap-2.5 px-6 sm:px-8 py-3.5 sm:py-4 bg-white text-slate-900 hover:bg-slate-100 font-semibold text-xs sm:text-sm uppercase tracking-wider transition-colors duration-200 shadow-sm"
                >
                ...
                {onOpenRegister && (
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-6 sm:px-8 py-3.5 sm:py-4 bg-white/10 hover:bg-white/20 backdrop-blur-md text-white border border-white/25 font-semibold text-xs sm:text-sm uppercase tracking-wider transition-colors duration-200"
                  >
