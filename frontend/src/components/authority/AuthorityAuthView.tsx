...
export function AuthorityAuthView({ onSuccess, onExitAuthority, onOpenRegisterPage }: AuthorityAuthViewProps) {
  const { signInAuthority, registerAuthority, resetPassword, loginAuthority } = useAuth();
  ...
  const handleInstantDemoLogin = (authority?: (typeof DEFAULT_DEMO_AUTHORITIES)[0]) => {
    clearMessages();
    const target = authority || DEFAULT_DEMO_AUTHORITIES[0];
    loginAuthority(target.role, target.displayName, target.email, target.department);
    onSuccess();
  };
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-between font-sans-ui selection:bg-amber-500/30">
      ...
      <main className="flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-lg rounded-2xl border border-slate-800 bg-slate-900/90 shadow-2xl p-6 sm:p-8 space-y-6">
          ...
          {/* Feedback Alerts */}
          {errorMsg && (
            <div className="p-3.5 rounded-xl bg-amber-950/40 border border-amber-800/80 text-xs text-amber-200 space-y-2.5 animate-in fade-in">
              ...
              {(errorMsg.toLowerCase().includes('firebase') || errorMsg.toLowerCase().includes('not enabled') || errorMsg.toLowerCase().includes('operation-not-allowed')) && (
                <div className="p-2.5 rounded-lg bg-slate-900/90 border border-slate-800 text-[11px] space-y-1.5 text-slate-300">
                  <p className="font-semibold text-amber-300">
                    To enable Email/Password authentication in Firebase Console:
                  </p>
                  ...
                  <div className="pt-1 flex flex-wrap items-center gap-3">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-950 bg-amber-400 hover:bg-amber-300 px-2 py-0.5 rounded transition-colors"
                    >
