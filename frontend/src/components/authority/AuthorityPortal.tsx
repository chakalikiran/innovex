...
export function AuthorityPortal({
  ...
  if (!authorityUser) {
    return (
      <AuthorityAuthView
        onSuccess={() => {}}
        onExitAuthority={onExitAuthority}
        onOpenRegisterPage={onOpenRegisterPage}
      />
    );
  }
  ...
  const portalTitle = authorityUser?.role === 'library_officer'
    ? 'Library Authority Portal'
    : authorityUser?.role === 'hostel_officer'
    ? 'Hostel Authority Portal'
    : authorityUser?.role === 'sports_officer'
    ? 'Sports Authority Portal'
    : authorityUser?.role === 'accounts_officer'
    ? 'Accounts Authority Portal'
    : 'Full Authority Portal';
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      await updateDepartmentAction(
        ...
        officerName,
        authorityUser?.email || 'officer@university.edu',
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-[10px] font-mono uppercase text-[var(--text-muted)]">
                  {isAdmin ? 'University-Wide Administration' : `${currentDeptMeta.name} Clearance Desk`}
                </span>
              </div>
            </div>
          </div>
